...
public class IngestSeqDriver extends CompaniesDriver {
  ...
  public int validate() throws IOException {
    ...
    if (!HDFSClientUtil.canDoAction(hdfsFileSystem, UserGroupInformation.getCurrentUser().getUserName(),
      ...
      if (log.isErrorEnabled()) {
        log.error("HDFS input directory [" + hdfsInputDir + "] has too restrictive permissions to read as user ["
    ...
    if (!HDFSClientUtil.canDoAction(hdfsFileSystem, UserGroupInformation.getCurrentUser().getUserName(),
      ...
      if (log.isErrorEnabled()) {
        log.error("HDFS output directory [" + hdfsOutputDir + "] has too restrictive permissions to write as user ["
            + UserGroupInformation.getCurrentUser().getUserName() + "]");
      }
      return CompaniesDriver.RETURN_FAILURE_INVALID_ARGS;
    }
    ...
    while (inputFiles.hasNext()) {
      ...
      if (fileStatus.getPath().getName().equals(CONF_MR_FILECOMMITTER_SUCCEEDED_FILE_NAME)
        ...
        String fileSuccessParentSuffix = fileSuccessParent
            .substring(fileSuccessParent.indexOf(hdfsInputPath.toString()) + hdfsInputPath.toString().length());
    ...
    for (Path inputPath : hdfsInputDirs) {
      Path ouputPath = new Path(hdfsOutputDir, IngestUtil.getNamespacedPath(Counter.RECORDS_VALID, inputPath.toString()
          .substring(inputPath.toString().indexOf(hdfsInputPath.toString()) + hdfsInputPath.toString().length() + 1)));
    ...
    if (ouputDirsExist) {
      if (log.isErrorEnabled()) {
        log.error("HDFS output directory [" + hdfsOutputDir + "] contains sub output directoires that already exist");
      }
      return CompaniesDriver.RETURN_FAILURE_INVALID_ARGS;
    }
  ...
  public int execute() throws IOException, InterruptedException, ClassNotFoundException {
    ...
    incramentCounter(IngestSeqDriver.class.getCanonicalName(), Counter.DATASETS,
        hdfsInputDirs.size() + hdfsSkippedDirs.size());
