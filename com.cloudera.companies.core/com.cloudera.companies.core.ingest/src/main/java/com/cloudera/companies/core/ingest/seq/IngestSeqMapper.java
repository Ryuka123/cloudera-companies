...
public class IngestSeqMapper extends Mapper<Text, Text, CompaniesFileKey, Text> {
	...
	protected void map(Text key, Text value, Context context) throws IOException, InterruptedException {
		...
		if (recordTokens.length != CompaniesFileMetaData.FILE_FIELDS_NUMBER || recordTokens[0].length() == 0) {
			context.getCounter(Counter.RECORDS_MALFORMED).increment(1);
			context.write(new CompaniesFileKey(Counter.RECORDS_MALFORMED, key.toString(), recordTokens[0]), new Text(value));
			System.err.println(recordTokens[0]);
		} else {
