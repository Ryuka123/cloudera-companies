...
public class IngestZipDriver extends CompaniesDriver {
  ...
  public int validate() throws IOException {
    ...
    if (!localInputPath.canExecute()) {
      if (log.isErrorEnabled()) {
        log.error("Local input directory [" + localInputDir + "] has too restrictive permissions to read as user ["
    ...
    if (hdfs.exists(hdfsOutputPath)) {
      ...
      if (!HDFSClientUtil.canDoAction(hdfs, UserGroupInformation.getCurrentUser().getUserName(), UserGroupInformation
          .getCurrentUser().getGroupNames(), hdfsOutputPath, FsAction.ALL)) {
        if (log.isErrorEnabled()) {
  ...
  public int execute() throws Exception {
    ...
    for (File localInputFile : FileUtils.listFiles(localInputPath, new String[] { "zip" }, true)) {
      if (localInputFile.isFile() && localInputFile.canRead()) {
        try {
          CompaniesFileMetaData companiesFileMetaData = CompaniesFileMetaData.parsePathZip(localInputFile.getName(),
              localInputFile.getParent());
          FileCopy fileCopy = new FileCopy(new Path(companiesFileMetaData.getName()), new Path(
              companiesFileMetaData.getDirectory()), new Path(hdfsOutputPath, companiesFileMetaData.getGroup()),
              companiesFileMetaData.getGroup(), new FileCopyCallback() {
                @Override
                public void afterCall(FileCopy that) throws IOException {
                  if (that.mode.equals(FileCopyMode.EXECUTE) && that.status.equals(FileCopyStatus.SUCCESS)) {
                    fileCopyByGroup.get(that.group).remove(that);
                    if (fileCopyByGroup.get(that.group).size() == 0) {
                      getFileSystem().create(
                          new Path(that.toDirectory, CompaniesDriver.CONF_MR_FILECOMMITTER_SUCCEEDED_FILE_NAME));
                      if (log.isInfoEnabled()) {
                        log.info("File group [" + that.group + "] successfully written to [" + that.toDirectory + "]");
                      }
                    }
                  }
                }
          ...
          if ((fileCopys = fileCopyByGroup.get(companiesFileMetaData.getGroup())) == null) {
            fileCopyByGroup.put(companiesFileMetaData.getGroup(), (fileCopys = new CopyOnWriteArraySet<FileCopy>()));
          }
    ...
    ExecutorService copyFileExector = new ThreadPoolExecutor(numberThreads, numberThreads, 0L, TimeUnit.MILLISECONDS,
    ...
    for (Future<FileCopy> fileCopyFuture : fileCopyFutures) {
      ...
      if (fileCopyFuture.isCancelled() || !fileCopy.status.equals(FileCopyStatus.SUCCESS)) {
        ...
        if (!fileCopy.call().status.equals(FileCopyStatus.SUCCESS)) {
          if (log.isErrorEnabled()) {
            log.error("Rollback failed for local input file [" + new Path(fileCopy.fromDirectory, fileCopy.fromFile)
                + "] to HDFS output file [" + new Path(fileCopy.toDirectory, fileCopy.fromFile) + "]");
          }
        }
        fileCopySuccess.remove(fileCopy);
        fileCopyFailure.add(fileCopy);
    ...
    incramentCounter(IngestZipDriver.class.getCanonicalName(), Counter.FILES,
        fileCopySuccess.size() + fileCopySkip.size() + fileCopyFailure.size());
    incramentCounter(IngestZipDriver.class.getCanonicalName(), Counter.FILES_SUCCESS, fileCopySuccess.size());
    incramentCounter(IngestZipDriver.class.getCanonicalName(), Counter.FILES_SKIP, fileCopySkip.size());
    incramentCounter(IngestZipDriver.class.getCanonicalName(), Counter.FILES_FAILURE, fileCopyFailure.size());
    ...
    public FileCopy call() throws Exception {
      ...
      if (log.isInfoEnabled()) {
        log.info("File copy mode [" + mode + "] returned [" + status + "] during ingest of local input file ["
            + new Path(fromDirectory, fromFile) + "] to HDFS output file [" + new Path(toDirectory, fromFile) + "]");
      }
      return this;
    }
